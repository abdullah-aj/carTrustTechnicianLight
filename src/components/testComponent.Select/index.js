...
import RenderQuestionScore from './renderQuestionScore';
import AP from '../../api';
...
class TestQuestionSelect extends React.Component<Props, State> {
  ...
  handleAnswersAction: function = async (answerType) => {
    if (answerType === ANSWER_NOT_APPLICABLE) {
      ...
      let score = 0;
      let allSame = true;
      let type = this.props.questionData.question.step_type; // TEST_TYPE_SELECT;
      this.state.selectedValueArr.forEach((element, i) => {
        if (element === 0) {
          hasError = true;
        ...
        if (element !== 0 && i > 0) {
          console.log(element.value, this.state.selectedValueArr[i - 1].value);
          if (element.value !== this.state.selectedValueArr[i - 1].value) {
            allSame = false;
          }
        }
      });
      if (hasError) {
        ...
        } else if (type === TEST_TYPE_SELECT_ALL) {
          const totalScore = allSame ? 10 : 0;

          this.props.onAnswer(
            TEST_TYPE_SELECT_ALL,
            JSON.stringify(this.state.selectedValueArr),
            totalScore,
          );
        } else {
          ...
          <View style={pageStyle.cardRowTwo}>
            ...
            <View
              ...
              {this.state.selectedValueArr.length > 0 && (
                <>
                  {(this.props.questionData.question.step_type ===
                    TEST_TYPE_SELECT ||
                    this.props.questionData.question.step_type ===
                      TEST_TYPE_SELECT_ALL) && (
                    <>
                      <RenderQuestion
                        ...
                        paused={this.state.buttonsDisable}
                      />
                    </>
                  )}
                  {this.props.questionData.question.step_type ===
