...
public class PrinterActionsActivity extends Activity implements OnClickListener {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		mTitle = (TextView) findViewById(R.id.statusText);
	...
	private void handleExternalCommands(){
		incomingExternalCommand = getIntent().getStringExtra(EXTERNAL_COMMAND_KEY);
		if (incomingExternalCommand != null && isPrinterConnected){
			if (incomingExternalCommand.equals(CMD_PRINT_QR_CODE)) {
				String text = getIntent().getStringExtra(EXTERNAL_DATA_VALUE_KEY);
				printQRCode(text);
			}else if (incomingExternalCommand.equals(CMD_PRINT_PICTURE)){
				String path = getIntent().getStringExtra(EXTERNAL_DATA_VALUE_KEY);
				if (path!=null) {
					Bitmap bitmap;
					if (path.startsWith("content://")){
						...
						bitmap = getBitmapFromBase64(path);
					}
					if (bitmap!=null){
	...
	private Bitmap getBitmapFromBase64(String encodedBase64Image){
		byte[] decodedString = Base64.decode(encodedBase64Image, Base64.DEFAULT);
	...
	public void onClick(View v) {
		...
		switch (v.getId()) {
			case R.id.button_scan:{
				Intent serverIntent = new Intent(PrinterActionsActivity.this, DeviceListActivity.class);
			...
			case R.id.Send_Button:{
				if (hexBox.isChecked()) {
					...
					if(str.length() > 0){
						...
						Toast.makeText(PrinterActionsActivity.this, getText(R.string.empty), Toast.LENGTH_SHORT).show();
					}
				} else {
			...
			switch (msg.what) {
				...
				case MESSAGE_DEVICE_NAME:
					...
					Toast.makeText(getApplicationContext(),
							...
							Toast.LENGTH_SHORT).show();
					isPrinterConnected = true;//New Code
					break;
				case MESSAGE_TOAST:
				...
				case MESSAGE_CONNECTION_LOST:    //蓝牙已断开连接
					Toast.makeText(getApplicationContext(), "Device connection was lost",
							Toast.LENGTH_SHORT).show();
					isPrinterConnected = false;//New Code
					editText.setEnabled(false);
					imageViewPicture.setEnabled(false);
					width_58mm.setEnabled(false);
					width_80.setEnabled(false);
...
/****************************************************************************************************/
	...
	private void printBarCode() {
		...
		new AlertDialog.Builder(PrinterActionsActivity.this).setTitle(getText(R.string.btn_prtcode))
					...
					public void onClick(DialogInterface dialog, int which) {
						...
						if(which == 0)
						{
							if(str.length() == 11 || str.length() == 12)
							{
								...
								Toast.makeText(PrinterActionsActivity.this, getText(R.string.msg_error), Toast.LENGTH_SHORT).show();
								return;
							}
						}
						else if(which == 1)
						...
						else if(which == 9)
						{
							if(str.length() == 0)
							{
								Toast.makeText(PrinterActionsActivity.this, getText(R.string.empty1), Toast.LENGTH_SHORT).show();
								return;
							}
							else
							{
	...
	private void CommandTest(){
		...
		String lang = getString(R.string.strLang);
		if((lang.compareTo("cn")) == 0){
			new AlertDialog.Builder(PrinterActionsActivity.this).setTitle(getText(R.string.chosecommand))
...
/****************************************************************************************************/
	...
	/*AlhamraniECR alhamraniECR = new AlhamraniECR(getApplicationContext(),getPackageName());
		private void initaMPOS(){
			List<BluetoothDevice> devices = alhamraniECR.getPairedDevices();
			if (devices!=null && devices.size()>0){
				boolean res = alhamraniECR.connectToDevice(devices.get(0));
				if (res)
					new ERCTransactions().execute();
				else
					Toast.makeText(PrinterActionsActivity.this,"Failed to connect. Please try again.",Toast.LENGTH_SHORT).show();
			}else {
				Toast.makeText(PrinterActionsActivity.this,"Please turn on the MPOS device and connect by bluetooth",Toast.LENGTH_SHORT).show();
			}
		}

		private class ERCTransactions extends AsyncTask<Void,Void,Object>{

			@Override
			protected Object doInBackground(Void... voids) {
				ECRResponse response = alhamraniECR.sendECRTransaction("123","1234567890","PUR"
						,"010","532416943","","","2");
				return response;
			}
			...
			protected void onPostExecute(Object o) {
				...
				if (o!=null && o instanceof ECRResponse){
					ECRResponse response = (ECRResponse) o;
					Toast.makeText(PrinterActionsActivity.this,"RESPONSE CODE: "+response.responseField.get(0).response_code+"\n"+
							"AMOUNT: "+response.responseField.get(0).amount,Toast.LENGTH_LONG).show();
				}
			}
		}*/
}